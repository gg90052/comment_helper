    ...
    <tbody>
      <template v-for="(tr, index) in sortTableData" :key="tr.id">
        <tr>
          <th>{{ index + 1 }}</th>
          <a
            :href="`https://www.facebook.com/${tr.from.id}`"
            class="text-[#4094c5]"
            target="_blank"
            >{{ username(tr) }}</a
          >
          <td class="w-3/5 whitespace-normal text-[#D68927] hover:underline">
            <a :href="'https://www.facebook.com/' + tr.id" target="_blank">{{
              tr.message ? tr.message : tr.id
            }}</a>
          </td>
          <td class="text-center">{{ tr.like_count }}</td>
          <td>{{ tr.created_time }}</td>
        </tr>
      </template>
    </tbody>
  </table>
