<template>
  <div class="flex-grow flex-shrink-0 max-w-lg text-left mobile:text-center mobile:max-w-full mobile:w-full">
    ...
    <div class="mt-4 flex flex-nowrap items-end mobile:justify-center" v-if="post.id !== ''">
      ...
      <button :class="isMobile() ? 'hidden':''" class="bg-blue-500 hover:bg-blue-400 transition-colors px-3 py-1 text-white font-bold rounded-md btn-free-height mr-2" @click="getData('shares')">
        <svg class="svg-icon fill-current w-10 mx-2" viewBox="0 0 20 20">
      ...
      <div class="text-center">
        <button @click="importShare" :class="isMobile() ? 'hidden':''" class="bg-blue-500 hover:bg-blue-400 transition-colors px-3 py-1 text-white rounded-md font-bold btn-sm block">導入資料</button>
